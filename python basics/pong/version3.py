...
class Game:
    ...
    def handle_key_down(self):
        ...
        # Handle each keydown event by moving a paddle
        # appropriately.
        # - self is the Game whose events will be handled
        keys = pygame.key.get_pressed()
        paddle_movement = 10
        if keys[K_q]:
            self.paddle_up(self.p_left, paddle_movement)
        if keys[K_a]:
            self.paddle_down(self.p_left, paddle_movement)
        if keys[K_p]:
            self.paddle_up(self.p_right, paddle_movement)
        if keys[K_l]:
            self.paddle_down(self.p_right, paddle_movement)

    def paddle_down(self, paddle, paddle_increment):

        # Move a given paddle down the window without letting
        # it move past the edge of the window.
        # - self is the Game
        # - paddle is the paddle to be moved down
        # - paddle_increment is the distance to move the paddle

        paddle.bottom = paddle.bottom + paddle_increment
        if paddle.bottom > self.window.get_height():
            paddle.bottom = self.window.get_height()

    def paddle_up(self, paddle, paddle_increment):

        # Move a given paddle up the window without letting
        # it move past the edge of the window.
        # - self is the Game
        # - paddle is the paddle to be moved up
        # - paddle_increment is the distance to move the paddle

        paddle.top = paddle.top - paddle_increment
        if paddle.top < 0:
            paddle.top = 0
    ...
    def draw_score(self):
        ...
        # Draw the left score and the right score.
        # - self is the Game
